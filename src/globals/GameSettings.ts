          ...
          fields: [
            {
              ...
              label: 'MBTI 個性穿搭測驗設定',
              fields: [
                { name: 'pointsCostPerPlay', label: '首次測驗消耗點數', type: 'number', defaultValue: 50, admin: { description: '會員首次測驗的基礎費用；重測費用會自動隨次數倍增' } },
                { name: 'retakeMultiplier', label: '重測費用成長倍率', type: 'number', defaultValue: 1.5, admin: { description: '每次重測費用 = 基礎 × 倍率^已測次數。例：基礎 50、倍率 1.5 → 第 1 次 50 / 第 2 次 75 / 第 3 次 113 / 第 4 次 169 / 第 5 次 253。設為 1 = 永遠收基礎價；設為 0/負數 = 視為 1' } },
                { name: 'dailyLimit', label: '每日次數上限', type: 'number', defaultValue: 1, admin: { description: '0 = 不限制；用於擋日刷' } },
                { name: 'allowRetake', label: '允許重複測驗', type: 'checkbox', defaultValue: true, admin: { description: '✅ 預設開啟 — 重測收 retakeMultiplier 倍費用，新結果覆寫舊 mbtiProfile。關閉則終身限 1 次' } },
                ...
                { name: 'description', label: '遊戲說明', type: 'textarea', defaultValue: '32 題專業 MBTI64 測驗，找出你的個性 + 場合穿搭風格。每次重測費用會略增，新結果會覆蓋舊紀錄。' },
                { name: 'icon', label: '圖示 Emoji', type: 'text', defaultValue: '🧠' },
              ],
            },
          ],
