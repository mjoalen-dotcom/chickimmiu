          ...
          fields: [
            {
              ...
              fields: [
                ...
                { name: 'dailyLimit', label: '每日次數上限', type: 'number', defaultValue: 1, admin: { description: '0 = 不限制；當「允許重複測驗」關閉時此欄無作用（一次性限制優先）' } },
                { name: 'allowRetake', label: '允許重複測驗', type: 'checkbox', defaultValue: true, admin: { description: '開啟 = 會員可重測，仍受「每日次數上限」約束。關閉 = 每位會員只能測 1 次' } },
                ...
                { name: 'description', label: '遊戲說明', type: 'textarea', defaultValue: '28 題專業 MBTI 測驗，找出你的個性穿搭風格！' },
                { name: 'icon', label: '圖示 Emoji', type: 'text', defaultValue: '🧠' },
              ],
            },
          ],
