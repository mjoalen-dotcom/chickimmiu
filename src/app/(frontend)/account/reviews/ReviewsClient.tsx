...
const STATUS_LABEL: Record<string, { label: string; color: string }> = {
  approved: { label: '已審核', color: 'text-green-600' },
  pending: { label: '審核中', color: 'text-yellow-600' },
  rejected: { label: '未通過', color: 'text-red-500' },
}
    ...
    <main className="space-y-8">
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-20 text-muted-foreground">
          ...
          <p className="mb-1">尚未有評價</p>
          <p className="text-xs">購物完成後可在此撰寫商品評價</p>
        </div>
      ) : (
        <div className="space-y-4">
          {reviews.map((review) => {
            const st = STATUS_LABEL[review.status] ?? STATUS_LABEL.pending
            const imgUrl = normalizeMediaUrl(review.productImage ?? undefined) || review.productImage

            return (
              <div key={review.id} className="bg-white rounded-2xl border border-cream-200 p-5">
                <div className="flex gap-4">
                  <div className="w-16 h-20 rounded-lg overflow-hidden relative shrink-0 border border-cream-200 bg-cream-100">
                    {imgUrl && (
                      <Image
                        src={imgUrl}
