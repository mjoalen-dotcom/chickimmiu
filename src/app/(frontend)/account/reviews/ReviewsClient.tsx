...
export function ReviewsClient({ reviews }: { reviews: ReviewLite[] }) {
    ...
    <main className="space-y-8">
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-lg font-serif mb-2">尚無評價</p>
          <p className="text-sm text-muted-foreground">購買商品後即可留下您的評價</p>
        </div>
      ) : (
            ...
            <div key={review.id} className="bg-white rounded-2xl border border-cream-200 p-5">
              <div className="flex gap-4">
                {review.productImage ? (
                  ...
                  <div className="w-16 h-20 rounded-lg shrink-0 border border-cream-200 bg-cream-100 flex items-center justify-center text-2xl">
                    👗
                  </div>
                )}
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    ...
                    >
                      {review.status === 'approved' ? '已發布' : review.status === 'rejected' ? '已拒絕' : '審核中'}
                    </span>
                  </div>
