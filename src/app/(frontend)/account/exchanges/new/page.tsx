...
import { ExchangeForm, type OrderLineLite } from './ExchangeForm'
...
export default async function NewExchangePage({
  ...
  if (!user) redirect(`/login?redirect=/account/exchanges/new?orderId=${orderId}`)
    ...
    <main className="space-y-6 animate-fade-in">
      <div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className="text-xl font-serif">申請換貨</h2>
      ...
      {!eligible ? (
        <div className="bg-white rounded-2xl border border-cream-200 p-6 text-sm text-muted-foreground">
          此訂單目前為「{status || '未知狀態'}」，僅已出貨或已送達的訂單可申請換貨。
        </div>
      ) : (
        <ExchangeForm
