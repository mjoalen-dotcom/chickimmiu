  ...
  return (
    <div className="space-y-3">
      ...
      {providers.facebook && (
        ...
        <button type="button" onClick={() => signIn('facebook', { callbackUrl: `/api/auth/bridge?next=${encodeURIComponent(redirectTo)}` })} className={btnClass}>
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          已有會員？請先用原方式登入，再到<Link className="underline underline-offset-2" href="/account/settings">帳號設定連結 Facebook</Link>，以保留原帳號的訂單與點數。
        </p>
        </div>
      )}
