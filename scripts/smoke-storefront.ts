/**
 * Storefront smoke test — Wave 1 PR-θ
 *
 * 對本機或 prod 抽樣 ~30 個路徑，驗 HTTP status + 關鍵字。
 *
 * Usage:
 *   pnpm smoke:storefront                          # 對 localhost:3006
 *   BASE_URL=https://pre.chickimmiu.com pnpm smoke:storefront
 *   FAIL_FAST=1 pnpm smoke:storefront              # 第一個失敗立刻退
 */
...
async function buildCases(): Promise<Case[]> {
  ...
  const cases: Case[] = [
    ...
    {
      name: '404-check',
      path: '/products/definitely-does-not-exist-xyz',
      expectStatus: 404,
    },
  ]
...
async function main() {
  ...
  for (const c of cases) {
    try {
      ...
      results.push({ name: c.name, ok: false, status: 0, reason: msg })
      console.log(`  ✗ ${c.name} → ERR ${msg}`)
      if (FAIL_FAST) break
    }
  }

  const failed = results.filter((r) => !r.ok)
  console.log(`\n${t(`results: ${results.length - failed.length}/${results.length} pass`)}`)
